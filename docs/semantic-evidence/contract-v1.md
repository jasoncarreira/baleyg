...
8. **Dispatch and freshness.** A has c0 with a fresh resolved internal virtual binding and one possible target V: `E(c0,A,null,boundary,dispatch)`. A also has c1 with an exact direct internal binding whose proof is `possiblyStale` because the requested producer version differs from the captured one: `E(c1,A,null,boundary,stale)`. `nodes=[N(A,0)]`; edges are those two in source order; `frontier=[]`; calls=2. Neither V nor c1's target is admitted; possible dispatch remains only in binding; partial is true.
