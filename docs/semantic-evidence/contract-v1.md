...
| `SyntaxId` | `sid:v1:` followed by the declaration digest defined below. | Wrong shape/digest is invalid. |
| `OccurrenceId` | `occ:v1:` followed by the occurrence digest defined below. | Wrong shape/digest is invalid. |
...
| Record.field | Type / nullability | Invariant | Failure or disposition |
|---|---|---|---|
| `SemanticBasis.producerId` | `Text` | Matches semantic producer. | Mismatch invalidates semantic artifact. |
| `.producerVersion` | `Text` | Exact captured version. | Missing value means basis cannot be fresh. |
| `.producerHash` | `Hash` | Captured executable digest. | Missing/mismatch means possibly stale; malformed hash rejects artifact. |
| `.artifactHash` | `Hash` | Digest of declared captured semantic artifact bytes, empty prefix. | Mismatch invalidates artifact. |
| `.language` | `Language` | Matches facts and producer. | Mismatch invalidates artifact. |
| `.sourceSetId` | `Text` | Matches requested/admitted source set. | Mismatch invalidates artifact. |
| `.revisionId` | `Text` | Captured semantic revision identity. | Mismatch participates in freshness; malformed/dangling identity rejects artifact. |
| `.sourceManifestHash` | `Hash` | Digest, empty prefix, of canonical JSON array `{document:DocumentKey,contentHash:Hash}` sorted language then path. | Missing/mismatch prevents use; malformed digest rejects artifact. |
| `.toolchainHash` | `Hash` | Digest of declared captured bytes, empty prefix. | Changed/unavailable is possibly stale. |
| `.configHash` | `Hash` | Digest of declared captured bytes, empty prefix. | Changed/unavailable is possibly stale. |
| `.dependencyHash` | `Hash` | Digest of declared captured bytes, empty prefix. | Changed/unavailable is possibly stale. |
| `.lookupDependencies` | `Text[]` | Sorted unique language lookup keys, never stable-ID spellings. | Duplicate/unsorted list invalidates basis; v1 does not use it for precision freshness. |
...
Freshness is independent from coverage:

1. Changed or missing source bytes for the evidence document is `stale`; its overlay is not applied to current syntax.
2. With identical document bytes, changed or unavailable semantic revision identity, source manifest, toolchain, config, dependency, producer version/hash, or any required basis component is `possiblyStale`. This is conservatively source-set-wide; lookup dependencies do not narrow it in v1.
3. Only a match of every required component is `fresh`. Native syntax needs matching measured bytes/revision and no invented semantic basis.
...
Join checks are concrete: a declaration-name anchor on exact bytes whose span equals only declaration `S1` is `{status:exact,candidateIds:[S1],diagnostic:null}` and can install a declaration binding. If two compatible producer records map to that same measured anchor, the join is `{status:ambiguous,candidateIds:[S1,S2],diagnostic:"two compatible declarations"}` and installs none. No compatible candidate gives `unmatched` with `[]` and a diagnostic; an unavailable position convention gives `unsupported` with `[]` and a diagnostic. A UTF-16 offset that splits a surrogate pair cannot be converted to a scalar-boundary UTF-8 range: it is an invalid location and atomically rejects the artifact, not an ambiguous or unsupported join.
...
Canonical JSON uses documented ASCII object keys sorted by ASCII byte value. Arrays retain their specified order unless explicitly a sorted set. Required nulls remain. Valid scalar strings are UTF-8 without normalization. Escape only quote, backslash, and U+0000–U+001F (as lowercase `\u00xx`); U+2028 and U+2029 remain literal UTF-8. Booleans/null use JSON literals and integers use minimal unsigned decimal. There is no whitespace, BOM, trailing newline, duplicate key, floating point, or negative zero. This is deliberately not RFC 8785.
...
Worked anchor audit (`H` is header hash and `G([...])` is the ordered sibling-group hash; each row checks old ID presence, header, duplicate rule, counts/hash/continuity, then result):

| Scenario | Captured fields | Current group / checks | Result |
|---|---|---|---|
| Unique body edit | `id=S0,H=H0,group=G([H0]),count=1,identical=1` | `S0` exists; header `H0`; body is unprojected; unique. | `{attached,S0,none}`. |
| Duplicate-header body edit, proven unchanged | Focus is first: `S0,H0,G([H0,H0]),2,2` | `S0` and `H0` match; current `G([H0,H0]),2,2`; continuity `r1→r2 unchanged` with evidence. | `{attached,S0,none}`. |
| Unique sibling inserted before | Old `S0,H0,G([H0]),1,1`; insertion has different header `Hx` but the same key, shifting the focused member to ordinal 1. | The inserted ordinal-0 declaration has old descriptor ID `S0` (header is excluded from the ID) but header `Hx≠H0`; the old member now has a different ID. | `{orphaned,null,headerMismatch}`; never follow the old member to its new ID. |
| Different-header sibling inserted after unique member | `S0,H0,G([H0]),1,1` | `S0` remains at ordinal 0 with `H0`; current group `G([H0,Hx]),2`, identical `H0` count 1. Unique rule ignores group membership change. | `{attached,S0,none}`. |
| Duplicate count changes | `S0,H0,G([H0,H0]),2,2` | `S0/H0` may match but current group/count differs, e.g. `G([H0,H0,H0]),3,3`. | `{orphaned,null,groupChanged}`. |
| Same-count reorder/replacement, continuity unknown | `S0,H0,G([H0,H0]),2,2`; current still shows the same two indistinguishable projected hashes/counts. | Equal bytes/counts do not prove membership/order; continuity is `unknown` with null evidence. | `{orphaned,null,unprovenContinuity}`. |
...
1. **Source order versus evaluation.** Owner A has outer `c0:[10,20)` to F and nested `c1:[12,18)` to G. `nodes=[N(A,0),N(F,1),N(G,1)]`; `edges=[E(c0,A,F,new,none),E(c1,A,G,new,none)]`; `frontier=[]`; calls=2, nodes=3. The outer start sorts first despite evaluation intuition. A callback body owned by C is not in A's call array merely because C's name is an argument; only an actual scheduler call is emitted.
2. **Cycle.** A has `a0→B`; B has `b0→A`; depth 2. `nodes=[N(A,0),N(B,1)]`; `edges=[E(a0,A,B,new,none),E(b0,B,A,seen,none)]`; `frontier=[]`; calls=2, nodes=2. A is never re-enqueued. Self-loop A→A: `nodes=[N(A,0)]`; `edges=[E(a0,A,A,seen,none)]`; `frontier=[]`; calls=1.
3. **Queued/visited diamond.** A local calls are `a0→B,a1→C`; B has `b0→D`; C has `c0→D`; depth 3. Admission/dequeue yields `nodes=[N(A,0),N(B,1),N(C,1),N(D,2)]`; `edges=[E(a0,A,B,new,none),E(a1,A,C,new,none),E(b0,B,D,new,none),E(c0,C,D,seen,none)]`; `frontier=[]`; calls=4. C sees D while D is queued, before D expansion.
4. **Depth.** Chain A→B→C, depth 1: `nodes=[N(A,0),N(B,1)]`; `edges=[E(a0,A,B,new,none)]`; `frontier=[F(depth,B,null,null,0,1)]`; calls=1; truncated/partial true. Depth 0: `nodes=[N(A,0)]`; `edges=[]`; `frontier=[F(depth,A,null,null,0,1)]`; calls=0.
5. **Node cap with later seen edge.** A calls in order `a0→B,a1→C,a2→A`, maxNodes=2. `nodes=[N(A,0),N(B,1)]`; `edges=[E(a0,A,B,new,none),E(a1,A,null,boundary,nodeLimit),E(a2,A,A,seen,none)]`; `frontier=[F(nodeLimit,A,a1,C,null,0)]`; calls=3. The refused C does not stop the later self-loop.
6. **Global call cap.** A calls `a0→B,a1→C`; B calls `b0→D`; maxCalls=1. After a0: `nodes=[N(A,0),N(B,1)]`; `edges=[E(a0,A,B,new,none)]`; `frontier=[F(callLimit,A,null,null,1,1),F(callLimit,B,null,null,0,1)]`; calls=1. The first frontier is created at A, then B is drained in queue order. With maxCalls=0 and root calls `[a0,a1]`: `nodes=[N(A,0)]`; `edges=[]`; `frontier=[F(callLimit,A,null,null,0,2)]`; calls=0. With maxCalls=1 and A's only call `a0→B`, B empty: arrays are nodes A,B, one new edge, `frontier=[]`; exact cap has no remaining work, so not truncated.
7. **Dispatch and freshness.** A has c0 with a fresh resolved internal virtual binding and one possible target V: `E(c0,A,null,boundary,dispatch)`. A also has c1 whose caller evidence is fresh but internal direct target bytes changed, `staleTarget=true`: `E(c1,A,null,boundary,stale)`. `nodes=[N(A,0)]`; edges are those two in source order; `frontier=[]`; calls=2. Neither V nor the stale target is admitted; possible dispatch remains only in binding; partial is true.
