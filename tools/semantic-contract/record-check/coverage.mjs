...
const key=(producerId,document,revisionId)=>encode([producerId,document.sourceSetId,document.language,document.path,revisionId]);
const documentKey=(document,revisionId)=>encode([document.sourceSetId,document.language,document.path,revisionId]);
const snapshotKey=(sourceSetId,revisionId)=>JSON.stringify([sourceSetId,revisionId]);
...
function capturedBasis(proof,loaded,proofRow){
 ...
 if(!document||document.contentHash!==proof.contentHash)fail('FRESHNESS.BASIS','contentHash','captured document bytes disagree');
 const producer=loaded.fixture.producers.find(x=>x.id===proof.producerId);
 if(!producer||!producer.languages.includes(proof.document.language)||!capture(loaded,'executable',producer.executableHash))fail('FRESHNESS.BASIS','producerId','captured executable or producer missing');
 if(producer.kind==='native'){
  if(proof.evidenceKind!=='measuredSyntax'||proof.basis!==null)fail('FRESHNESS.BASIS','basis','native proof requires null basis');
  return producer;
 }
 if(proof.evidenceKind==='measuredSyntax'||proof.basis===null)fail('FRESHNESS.BASIS','basis','semantic proof requires a basis');
 const basis=proof.basis,revision=loaded.revisions.get(snapshotKey(proof.document.sourceSetId,proof.revisionId));
 const expected={producerId:producer.id,producerVersion:producer.version,producerHash:producer.executableHash,language:proof.document.language,sourceSetId:proof.document.sourceSetId,revisionId:proof.revisionId,sourceManifestHash:sourceManifestHash(revision.documents.map(x=>({document:x.key,contentHash:x.contentHash}))),toolchainHash:revision.toolchainHash,configHash:revision.configHash,dependencyHash:revision.dependencyHash};
 for(const [field,value] of Object.entries(expected))if(basis[field]!==value)fail('FRESHNESS.BASIS',`basis.${field}`,'captured self-relation differs');
 for(const [field,kind] of [['toolchainHash','toolchain'],['configHash','config'],['dependencyHash','dependency']])if(!capture(loaded,kind,basis[field]))fail('FRESHNESS.BASIS',`basis.${field}`,'captured bytes missing or changed');
 if(!capture(loaded,'semanticArtifact',basis.artifactHash)||!loaded.semanticBytes.some(x=>x.value.producerId===producer.id&&contentHash(x.bytes)===basis.artifactHash))fail('FRESHNESS.BASIS','basis.artifactHash','artifact capture missing or changed');
 if(basis.lookupDependencies.some(x=>/^(?:sid|occ):v1:[a-f0-9]{32}$/.test(x))||basis.lookupDependencies.some((x,i)=>i&&order(basis.lookupDependencies[i-1],x)>=0))fail('FRESHNESS.BASIS','basis.lookupDependencies','lookup keys must be ordered, unique, and not identity IDs');
...
function coverageState(row,intent){
 ...
 if(row.observedRoles.some(x=>!effective.includes(x)))fail('COVERAGE.ROLES','coverage.observedRoles','role is not requested, supported and available');
 if(!row.requested)return 'notRequested';
 if(!row.selected){if(row.observedRoles.length)fail('COVERAGE.ROLES','coverage.observedRoles','unselected tuple cannot observe selected evidence');return effective.length?'omitted':'unsupported';}
...
export function checkCoverage(loaded,records){
 ...
 if(records.formatVersion!==loaded.fixture.formatVersion)fail('RECORDS.IDENTITY','formatVersion','version differs');
 requireEqual(records.comparison,loaded.fixture.comparison,'RECORDS.IDENTITY','comparison');
 const expectedProducers=unique(loaded.fixture.producers,x=>x.id,'RECORDS.IDENTITY','producers');
 const actualProducers=unique(records.producers,x=>x.id,'RECORDS.IDENTITY','producers');
 const expectedSets=unique(loaded.fixture.sourceSets,x=>x.id,'RECORDS.IDENTITY','sourceSets');
 const actualSets=unique(records.sourceSets,x=>x.id,'RECORDS.IDENTITY','sourceSets');
 const expectedRevisions=new Map();
 for(const revision of loaded.fixture.revisions){
  const source=loaded.revisions.get(snapshotKey(revision.sourceSetId,revision.id));
  if(!source)fail('RECORDS.IDENTITY','revisions','admitted snapshot missing');
  const documents=revision.documents.map(item=>{const bytes=loaded.sources.get(JSON.stringify([revision.sourceSetId,revision.id,item.key.path]));if(!bytes)fail('RECORDS.IDENTITY','revisions.documents','source bytes missing');return {key:item.key,revisionId:item.revisionId,contentHash:contentHash(bytes),byteLength:bytes.length};});
  expectedRevisions.set(snapshotKey(revision.sourceSetId,revision.id),{...revision,documents});
 }
 ...
 for(const producer of loaded.fixture.producers)for(const revision of expectedRevisions.values())for(const document of revision.documents){
  if(!producer.languages.includes(document.key.language))continue;
  const id=key(producer.id,document.key,revision.id),intent=intents.get(id);
  if(!intent)fail('COVERAGE.TUPLE','coverage','missing admitted intent');
  expected.set(id,{producer,revision,document,intent});
 }
 if(intents.size!==expected.size)fail('COVERAGE.TUPLE','coverage','extra or missing intent');
 const coverageByTuple=unique(records.coverage,x=>key(x.producerId,{sourceSetId:x.sourceSetId,language:x.language,path:x.documentPath},x.revisionId),'COVERAGE.TUPLE','coverage');
 ...
 for(const annotation of loaded.annotations)for(const fact of annotation.facts){
  ...
  const row=fact.record,id=key(row.producerId,{sourceSetId:row.sourceSetId,language:row.language,path:row.documentPath},row.revisionId);
  if(row.sourceSetId!==annotation.document.sourceSetId||row.language!==annotation.document.language||row.documentPath!==annotation.document.path||row.revisionId!==annotation.revisionId||authoredCoverage.has(id))fail('COVERAGE.TUPLE','coverage','coverage fact has wrong annotation tuple or duplicates it');
  authoredCoverage.set(id,row);
 }
 if(coverageByTuple.size!==expected.size||authoredCoverage.size!==expected.size)fail('COVERAGE.TUPLE','coverage','missing or extra coverage tuple');
 for(const [id,{intent}] of expected){
  const row=coverageByTuple.get(id);if(!row)fail('COVERAGE.TUPLE','coverage','missing coverage tuple');
  if(row.selected!==authoredCoverage.get(id)?.selected)fail('COVERAGE.STATE','coverage','selected flag differs from captured intent');
  sortedRoles(intent.requestedRoles,intent.document.language,'coverage.requestedRoles');
  sortedRoles(row.supportedRoles,row.language,'coverage.supportedRoles');sortedRoles(row.observedRoles,row.language,'coverage.observedRoles');
  const expectedRequested=intent.requestedRoles.length>0;
  if(row.requested!==expectedRequested||row.selected&&!row.requested)fail('COVERAGE.STATE','coverage','requested or selected contradicts intent');
  const state=coverageState(row,intent);
  if(row.state!==state||(row.diagnostic===null)!==['notRequested','complete'].includes(state))fail('COVERAGE.STATE','coverage',`state or diagnostic contradicts source evidence: ${row.producerId}/${row.revisionId}/${row.documentPath} ${row.state} expected ${state}`);
  const authored=authoredCoverage.get(id);
  if(!authored)fail('COVERAGE.TUPLE','coverage','coverage row not present in captured annotation');
  for(const field of ['requested','selected','state','supportedRoles','observedRoles','diagnostic'])if(!equal(row[field],authored[field]))
   fail(field.endsWith('Roles')?'COVERAGE.ROLES':'COVERAGE.STATE',field.endsWith('Roles')?`coverage.${field}`:'coverage','normalized coverage differs from captured claim');
 }
 const semanticProofsById=new Map();
 for(const annotation of loaded.annotations)for(const fact of annotation.facts){
  if(fact.kind!=='provenance'||fact.record.basis===null)continue;
  const row=fact.record;
  if(semanticProofsById.has(row.id)&&!equal(semanticProofsById.get(row.id),row))fail('FRESHNESS.BASIS','provenance','conflicting captured semantic proofs');
  semanticProofsById.set(row.id,row);
 }
 for(const row of semanticProofsById.values())capturedBasis(row,loaded,loaded.semanticProofs.get(row.id));
 const checkedProofs=new Map();
 for(const proof of records.provenance){
  if(checkedProofs.has(proof.id))fail('FRESHNESS.BASIS','provenance','duplicate proof ID');
  const producer=capturedBasis(proof,loaded,loaded.semanticProofs.get(proof.id));
  if(producer.kind==='semantic'){
   const raw=semanticProofsById.get(proof.id);
   if(!raw||!equal((({freshness,...captured})=>captured)(raw),(({freshness,...captured})=>captured)(proof)))fail('FRESHNESS.BASIS','provenance','not an admitted semantic proof');
  }
 ...
 function checkUse({producerId,document,revisionId,provenanceIds}){
  ...
  if(!Array.isArray(provenanceIds)||new Set(provenanceIds).size!==provenanceIds.length)fail('FRESHNESS.USE','provenanceIds','proof IDs repeated or invalid');
  const proofs=provenanceIds.map(proofId=>{
   const proof=checkedProofs.get(proofId);
   if(!proof||proof.producerId!==producerId||!equal(proof.document,document)||proof.revisionId!==revisionId||
 ...
 function checkTarget(binding,proof,verifiedDeclarations){
  ...
  if(binding.provenanceId!==proof?.id||!checkedProofs.has(proof.id))fail('FRESHNESS.TARGET','provenanceId','binding proof is not checked');
  const target=binding.declaredTarget;
  let staleTarget=null;
  if(target?.kind==='internal'){
   if(!(verifiedDeclarations instanceof Map))fail('FRESHNESS.TARGET','declaredTarget','verified declaration map required');
   const captured=documentAt(loaded,target.document,target.revisionId);
   const declaration=verifiedDeclarations.get(snapshotKey(target.document.sourceSetId,target.revisionId))?.get(target.syntaxId);
   if(!captured||!declaration||!equal(declaration,target.document))fail('FRESHNESS.TARGET','declaredTarget','target not present in captured declarations');
   const wanted=loaded.selected.documents.find(x=>equal(x.key,target.document));
   const current=verifiedDeclarations.get(snapshotKey(loaded.comparison.sourceSetId,loaded.comparison.revisionId))?.get(target.syntaxId);
   staleTarget=!wanted||wanted.contentHash!==captured.contentHash||!current||!equal(current,wanted.key);
  }
  if(binding.staleTarget!==staleTarget)fail('FRESHNESS.TARGET','staleTarget','target bytes or declaration disagree');
  return staleTarget;
 }
 return {comparison:records.comparison,producers:actualProducers,sourceSets:actualSets,revisions:actualRevisions,coverageByTuple,semanticProofsById,checkUse,checkTarget};
}
