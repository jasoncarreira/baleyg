...
function cardinality(value) {
  const valid =
    (value.resolution === "resolved" &&
      value.declaredTarget?.kind === "internal" &&
      !value.candidates.length) ||
    (value.resolution === "external" &&
      value.declaredTarget?.kind === "external" &&
      !value.candidates.length) ||
  ...
  if (!valid)
    reject(
      "BINDING.CARDINALITY",
      "resolution",
      "resolution and target cardinality differ",
    );
}
...
export function checkBindings(loaded, records, C, M, J) {
  ...
  for (const native of loaded.native.declarations) {
    const measured = M.recordByNativeRef.get(native.ref),
      snapshot = JSON.stringify([
        native.document.sourceSetId,
        native.revisionId,
      ]);
    if (
      !measured?.syntaxId ||
      !same(measured.document, native.document) ||
      measured.revisionId !== native.revisionId
    )
      reject(
        "BINDING.TARGET",
        "declaredTarget",
        "target declaration lacks measured identity",
      );
    if (!verifiedDeclarations.has(snapshot))
      verifiedDeclarations.set(snapshot, new Map());
    verifiedDeclarations
      .get(snapshot)
      .set(measured.syntaxId, measured.document);
  }
  const resolve = (ref, proof, field) => {
    ...
    if (ref.kind === "external") {
      if ((ref.symbol.scope === "global") !== (ref.symbol.document === null))
        reject(
          "BINDING.TARGET",
          field,
          "external symbol scope and document differ",
        );
      if (
        ref.symbol.scope === "document" &&
        !same(ref.symbol.document, proof.document)
      )
        reject(
          "BINDING.TARGET",
          field,
          "document symbol outside proven document",
        );
    ...
    if (
      ...
      reject(
        "BINDING.TARGET",
        field,
        "target must name measured declaration in its source set and revision",
      );
    return {
      kind: "internal",
  ...
  for (const annotation of loaded.annotations)
    for (const fact of annotation.facts) {
      ...
      if (seen.has(fact.ref))
        reject("BINDING.FACT", "factRef", "duplicate call fact reference");
      seen.add(fact.ref);
      const joined = J.joined.get(fact.ref),
        proof = proofById.get(fact.record.provenanceId);
      const captured = C.semanticProofsById.get(fact.record.provenanceId);
      const strip = ({ freshness, ...row }) => row;
      if (
        !proof ||
        !captured ||
        proof.evidenceKind !== "semanticReference" ||
        !same(strip(proof), strip(captured)) ||
        ...
        reject(
          "BINDING.FACT",
          "provenanceId",
          "call fact lacks its admitted proof and source tuple",
        );
      if (
        !joined ||
        ...
        !same(joined.join.anchor.document, proof.document) ||
        joined.join.anchor.revisionId !== proof.revisionId ||
      ...
      const exact = joined.join.status === "exact",
        call = exact
          ? loaded.native.calls.filter(
              (row) =>
                joined.nativeRefs.includes(row.ref) &&
      ...
      if (
        (exact &&
          (joined.join.candidateIds.length !== 1 ||
            ...
            call.length !== 1 ||
            !same(call[0].document, proof.document) ||
            call[0].revisionId !== proof.revisionId)) ||
        (!exact &&
          (joined.installedId !== null || joined.nativeRefs.length !== 0))
      )
        reject(
          "BINDING.JOIN",
          "join",
          "non-exact join installed a call or exact join lacks one measured call",
        );
      ...
      if (raw.possibleDispatchComplete !== false)
        reject(
          ...
          "possible dispatch is never complete",
        );
      if (
        ![
          "direct",
      ...
      const candidates = uniqueTargets(
        raw.candidates.map((x) => resolve(x, proof, "candidates")),
        "candidates",
      );
      const possibleDispatch = uniqueTargets(
        raw.possibleDispatch.map((x) => resolve(x, proof, "possibleDispatch")),
        "possibleDispatch",
      );
      const value = {
        callId: exact ? joined.installedId : null,
        join: joined.join,
      ...
      if (
        ...
        reject(
          "BINDING.CARDINALITY",
          "candidates",
          "ambiguous candidates must be sorted and unique",
        );
      if (declaredTarget?.kind === "internal") {
        const capturedTarget = loaded.revisions
          .get(
            JSON.stringify([
              declaredTarget.document.sourceSetId,
              declaredTarget.revisionId,
            ]),
          )
      ...
      const groupAnchor = exact
        ? null
        : { ...joined.join.anchor, ownerRef: fact.anchor.ownerRef };
      const groupKey = key([
        proof.producerId,
        proof.document,
        proof.revisionId,
  ...
  for (const group of groups.values()) {
    ...
    group.targetProofs = entries.map((x) => ({
      factRef: x.factRef,
      provenanceId: x.proof.id,
      declaredTarget: x.value.declaredTarget,
      candidates: x.value.candidates,
      possibleDispatch: x.value.possibleDispatch,
      proof: x.proof,
    }));
    group.historicalTuples = entries.map((x) => ({
      factRef: x.factRef,
      producerId: x.proof.producerId,
      document: x.proof.document,
      revisionId: x.proof.revisionId,
      freshness: x.proof.freshness,
    }));
    const unique = [
      ...new Map(entries.map((x) => [key(x.value), x.value])).values(),
    ];
    if (unique.length > 1) {
      const consistent = unique.every(
        (x) =>
          x.callId === first.callId &&
          same(x.join, first.join) &&
          x.dispatch === first.dispatch &&
          same(x.possibleDispatch, first.possibleDispatch),
      );
      if (!consistent)
        reject(
      ...
      if (contradiction) {
        if (
          ...
          reject(
            "BINDING.CONTRADICTION",
            "declaredTarget",
            "contradictory targets need distinct exact target proofs",
          );
    ...
    group.members = orderedEnvelope("callBindings", unique, {
      collapseIdentical: true,
    });
    expected.push(...group.members);
    for (const entry of entries) {
      const member = group.members.find(
        (x) => x.provenanceId === entry.proof.id,
      );
      if (!member)
        reject(
          "BINDING.CONTRIBUTORS",
          "provenanceId",
          "missing contributor proof member",
        );
  ...
  for (const member of expected) {
    const supplied = records.callBindings.find(
      (x) => x.provenanceId === member.provenanceId,
    );
    ...
    if (
      supplied.dispatch !== member.dispatch ||
      !same(supplied.possibleDispatch, member.possibleDispatch) ||
      supplied.possibleDispatchComplete !== false
    )
      reject("BINDING.DISPATCH", "dispatch", "output dispatch differs");
    if (
      supplied.resolution !== member.resolution ||
      !same(supplied.candidates, member.candidates) ||
      !same(supplied.declaredTarget, member.declaredTarget)
    )
      reject(
        member.resolution === "ambiguous" &&
          groupHasContradiction(groups, member)
          ? "BINDING.CONTRADICTION"
          : "BINDING.CARDINALITY",
        member.resolution === "ambiguous" ? "candidates" : "resolution",
        "output resolution or target union differs",
      );
  ...
  if (
    !same(
      orderedEnvelope("callBindings", expected, { collapseIdentical: true }),
      records.callBindings,
    )
  )
    reject(
      "BINDING.CONTRIBUTORS",
      "callBindings",
      "captured fact members and normalized binding inventory differ",
    );
  return {
    callBindings: orderedEnvelope("callBindings", expected, {
      collapseIdentical: true,
    }),
    recordByFactRef,
    groups,
...
function groupHasContradiction(groups, member) {
  return [...groups.values()].some(
    (group) =>
      ...
      group.targetProofs.some(
        (x) =>
          x.declaredTarget !== null &&
          group.targetProofs.some(
            (y) =>
              y.declaredTarget !== null &&
              !same(x.declaredTarget, y.declaredTarget),
          ),
      ),
  );
}
