...
function closure(field, expected, actual) {
  ...
  if (!same(orderedEnvelope(field, expected), actual))
    reject(
      "ANCHOR.MEMBERSHIP",
      field,
      "measured anchor inventory or result differs",
    );
}
function documentAt(loaded, document, revisionId) {
  return (
    loaded.revisions
      .get(JSON.stringify([document.sourceSetId, revisionId]))
      ?.documents.some((row) => same(row.key, document)) ?? false
  );
}
function groupFor(measurement, ref) {
  ...
  if (
    !group ||
    !group.memberRefs.length ||
    group.memberRefs.length !== group.headers.length ||
    !group.memberRefs.includes(ref)
  )
    reject(
      "ANCHOR.GROUP",
      "groupsByDeclarationRef",
      "missing measured sibling group",
    );
  const headers = group.memberRefs.map((member) => {
    ...
    if (!row?.header)
      reject(
        "ANCHOR.GROUP",
        "groupsByDeclarationRef",
        "missing measured sibling",
      );
    return measuredHeaderHash(row.header);
  });
  if (!same(headers, group.headers))
    reject(
      "ANCHOR.GROUP",
      "groupsByDeclarationRef",
...
export function checkAnchors(loaded, records, measurement) {
  if (
    ...
    reject(
      "ANCHOR.GROUP",
      "groupsByDeclarationRef",
      "verified measurement inventory required",
    );
  const declarations = loaded.native.declarations;
  const expectedAnchors = [],
    expectedContinuities = [],
    expectedResults = [];
  for (const anchorCase of loaded.anchors.cases) {
    const captured = declarations.find(
      (row) => row.ref === anchorCase.capturedDeclarationRef,
    );
    const currentRevisionId = anchorCase.currentRevisionId;
    if (
      !captured ||
      !documentAt(loaded, captured.document, captured.revisionId) ||
      ...
      reject(
        "ANCHOR.TUPLE",
        "continuity",
        "unadmitted document, revision or continuity",
      );
    if (
      (anchorCase.continuity.state === "unknown" &&
        anchorCase.continuity.evidence !== null) ||
      (currentRevisionId !== captured.revisionId &&
        anchorCase.continuity.state === "unchanged" &&
        (typeof anchorCase.continuity.evidence !== "string" ||
          !anchorCase.continuity.evidence.trim()))
    )
      reject(
        "ANCHOR.CONTINUITY",
        "continuity.evidence",
        "unknown cannot assert proof; cross-revision unchanged needs independent proof",
      );
    const old = measurement.recordByNativeRef.get(captured.ref);
    if (
      !old?.syntaxId ||
      old.revisionId !== captured.revisionId ||
      !same(old.document, captured.document)
    )
      reject(
        "ANCHOR.TUPLE",
        "capturedDeclarationRef",
        "missing captured measured declaration",
      );
    const capturedHeaders = groupFor(measurement, captured.ref);
    const headerHash = measuredHeaderHash(old.header);
    const durable = {
      syntaxId: old.syntaxId,
      document: old.document,
      capturedRevisionId: old.revisionId,
      headerHash,
      siblingGroupHash: measuredSiblingGroupHash(capturedHeaders),
      siblingCount: capturedHeaders.length,
      identicalHeaderCount: capturedHeaders.filter((x) => x === headerHash)
        .length,
    };
    ...
    const candidate = declarations.filter(
      (row) =>
        row.revisionId === currentRevisionId &&
        same(row.document, captured.document) &&
        measurement.recordByNativeRef.get(row.ref)?.syntaxId === old.syntaxId,
    );
    if (candidate.length > 1)
      reject("ANCHOR.TUPLE", "syntaxId", "ambiguous current measured identity");
    let reason = "none";
    if (!candidate.length) reason = "missing";
      ...
      if (currentHash !== headerHash) reason = "headerMismatch";
        ...
        if (durable.identicalHeaderCount > 1 || currentCount > 1) {
          if (
            durable.siblingGroupHash !== measuredSiblingGroupHash(headers) ||
            durable.siblingCount !== headers.length ||
            durable.identicalHeaderCount !== currentCount ||
            (currentRevisionId !== captured.revisionId &&
              anchorCase.continuity.state === "changed")
          )
            reason = "groupChanged";
          else if (
            currentRevisionId !== captured.revisionId &&
    ...
    const result = {
      status: reason === "none" ? "attached" : "orphaned",
      targetId: reason === "none" ? old.syntaxId : null,
      reason,
    };
    ...
    const targetId =
      typeof authored.targetId === "object" && authored.targetId !== null
        ? measurement.identityByRef.get(authored.targetId.ref)
        : authored.targetId;
    if (!same({ ...authored, targetId }, result))
      reject(
        "ANCHOR.RESULT",
        "expectedResult",
        "authored case differs from source-derived result",
      );
  }
  ...
  return {
    durableAnchors: expectedAnchors,
    groupContinuities: expectedContinuities,
    anchorResults: expectedResults,
  };
}
