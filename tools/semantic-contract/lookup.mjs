...
function decodeEscapes(spelling,language) {
  ...
  if (language==='java') return spelling.replace(/\\u+([0-9a-fA-F]{4})/g,(_,digits)=>String.fromCharCode(parseInt(digits,16)));
  const decoded=spelling.replace(/\\u(?:([0-9a-fA-F]{4})|\{([0-9a-fA-F]{1,6})\})/g,(_,fixed,variable)=> {
    const code=parseInt(fixed ?? variable,16);
    if (code>0x10ffff || (variable && code>=0xd800 && code<=0xdfff)) throw new SyntaxError('LOOKUP.ESCAPE invalid scalar');
...
export function lookupKey(language,spelling) {
  ...
  return language==='rust' ? decoded.normalize('NFC') : language==='python' ? decoded.normalize('NFKC') : decoded;
}
export function applicableRoles(language) {
  if (!languages.includes(language)) throw new TypeError('ROLE.LANGUAGE unknown language');
  return language==='java' ? roles.slice(0,-1) : [...roles];
}
export function validateRoles(language,selected,{site,callee=false}={}) {
