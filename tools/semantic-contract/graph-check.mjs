...
const tuple=(producer,document,revision)=>JSON.stringify([producer,documentKey(document),revision]);
...
export function expectedGraph(loaded,records,checked,request){
 ...
 for(const [name,defaultValue,min,max] of limits){
  ...
  if(!Number.isSafeInteger(effective[name])||effective[name]<min||effective[name]>max)return failure('invalidRequest',name);
 }
 if(typeof effective.sourceSetId!=='string'||!effective.sourceSetId||
    ...
    !(effective.semanticProducerId===null||typeof effective.semanticProducerId==='string'&&effective.semanticProducerId.length))
  return failure('invalidRequest','request');
 if(!records.sourceSets.some(s=>s.id===effective.sourceSetId))return failure('sourceSetDenied','sourceSetId');
 if(!records.revisions.some(r=>r.id===effective.revisionId&&r.sourceSetId===effective.sourceSetId))return failure('revisionUnavailable','revisionId');
 ...
 if(!byId.has(effective.rootSyntaxId))return failure('rootMissing','rootSyntaxId');
 ...
 for(const call of records.calls){
  ...
  const local=calls.get(call.ownerSyntaxId)??[];local.push(call);calls.set(call.ownerSyntaxId,local);
 }
 for(const local of calls.values())local.sort((a,b)=>bytes(a.document.path,b.document.path)||a.range.start-b.range.start||a.range.end-b.range.end||bytes(a.id,b.id));
 const bindings=new Map();
 if(effective.semanticProducerId!==null)for(const binding of records.callBindings){
  if(binding.callId===null||binding.join.status!=='exact'||binding.join.candidateIds.length!==1||binding.join.candidateIds[0]!==binding.callId)continue;
  const p=proofs.get(binding.provenanceId);
  if(p?.producerId!==effective.semanticProducerId||p.revisionId!==effective.revisionId||binding.join.anchor.revisionId!==effective.revisionId)continue;
  const local=bindings.get(binding.callId)??[];local.push(binding);bindings.set(binding.callId,local);
 }
 const nodes=[{declaration:byId.get(effective.rootSyntaxId),depth:0}],edges=[],frontier=[];
 const seen=new Set([effective.rootSyntaxId]);let head=0,callCapReached=false;
 while(head<nodes.length){
  ...
  if(depth===effective.depth||callCapReached){
   frontier.push({reason:depth===effective.depth?'depth':'callLimit',nodeId:declaration.syntaxId,callId:null,targetId:null,nextOrdinal:0,omittedCalls:local.length});
   continue;
  }
  for(let i=0;i<local.length;i++){
   const call=local[i];
   if(edges.length===effective.maxCalls){
    frontier.push({reason:'callLimit',nodeId:declaration.syntaxId,callId:null,targetId:null,nextOrdinal:call.ordinal,omittedCalls:local.length-i});
    callCapReached=true;break;
   }
   const row=coverage.get(tuple(effective.semanticProducerId,call.document,effective.revisionId));
   ...
   else if(proof.freshness!=='fresh'||binding.declaredTarget?.kind==='internal'&&binding.staleTarget!==false)reason='stale';
   ...
   if(reason==='none'&&(!target||target.kind!=='internal'||!byId.has(target.syntaxId)||
    documentKey(byId.get(target.syntaxId).document)!==documentKey(target.document)))reason='stale';
   let to=null,visit='boundary';
   if(reason==='none'){
    if(seen.has(target.syntaxId)){to=target.syntaxId;visit='seen';}
    else if(nodes.length===effective.maxNodes){
     reason='nodeLimit';frontier.push({reason:'nodeLimit',nodeId:declaration.syntaxId,callId:call.id,targetId:target.syntaxId,nextOrdinal:null,omittedCalls:0});
    }else{to=target.syntaxId;visit='new';seen.add(to);nodes.push({declaration:byId.get(to),depth:depth+1});}
   }
   edges.push({call,from:declaration.syntaxId,to,binding,visit,boundaryReason:reason});
  }
 }
 const draft={request:effective,resolvedRevisionId:effective.revisionId,nodes,edges,frontier};
 const evidence=selectGraphEvidence(loaded,records,checked,draft);
 const truncated=frontier.length>0;
 return {ok:true,result:{...draft,coverage:evidence.coverage,provenance:evidence.provenance,partial:truncated||evidence.selectedCoverageIncomplete||edges.some(edge=>edge.visit==='boundary'),truncated}};
}
...
export function checkAnswers(loaded,records,checked,materializedAnswers){
 ...
 for(const entry of cases){
  if(ids.has(entry.id))fail('GRAPH.CASES','answers','duplicate answer case');ids.add(entry.id);
...
export function checkGraphAnswer(loaded,records,checked,entry){
 const expected=expectedGraph(loaded,records,checked,entry.attemptedRequest);
  if(entry.answer.ok!==expected.ok)fail('GRAPH.ANSWER',`answers.${entry.id}.ok`,'answer outcome differs');
  if(!expected.ok){
   if(entry.answer.error.code!==expected.error.code)fail('GRAPH.ERROR',`answers.${entry.id}.error.code`,'wrong failure code');
   if(entry.answer.error.field!==expected.error.field)fail('GRAPH.ERROR',`answers.${entry.id}.error.field`,'wrong failure field');
   if(typeof entry.answer.error.message!=='string'||!entry.answer.error.message.trim())fail('GRAPH.ERROR',`answers.${entry.id}.error.message`,'empty error explanation');
   return true;
  }
  const actual=entry.answer.result,reference=expected.result;
  for(const field of ['request','resolvedRevisionId','nodes','edges','frontier','partial','truncated'])
   if(!equal(actual[field],reference[field]))fail('GRAPH.TRAVERSAL',`answers.${entry.id}.result.${field}`,'authored graph differs from pinned source-derived traversal');
