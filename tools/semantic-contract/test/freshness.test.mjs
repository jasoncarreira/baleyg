...
test('FRESHNESS.COVERAGE keeps partial fresh, complete stale and failed refresh historical',async t=>{
 ...
 const coverage={producerId:'semantic',language:'javascript',sourceSetId:'main',documentPath:s.document.path,revisionId:'r1',requested:true,selected:true,state:'complete',supportedRoles:['call'],observedRoles:['call'],diagnostic:null};
 const capturedAnnotation=JSON.parse(s.files['src/go.js.annotations.json']);
 capturedAnnotation.facts.push({kind:'coverage',ref:'r1-coverage',record:coverage});
 ...
 const failedCoverage={...coverage,revisionId:'r2',state:'failed',observedRoles:[],diagnostic:'semantic refresh failed'};
 s.files['r2/go.js.annotations.json']=JSON.stringify({formatVersion:1,document:s.document,revisionId:'r2',scenarios:[],facts:[{kind:'coverage',ref:'r2-failed-coverage',record:failedCoverage}]});
 ...
 assert.ok(loaded.fixture.coverageIntents.find(x=>x.revisionId==='r2')?.measurementSupport.every(x=>x.available));
 const rows=loaded.annotations.flatMap(x=>x.facts.filter(f=>f.kind==='coverage').map(f=>f.record));
 assert.equal(JSON.stringify(rows.find(x=>x.revisionId==='r1')),JSON.stringify(coverage));
 const failed=rows.find(x=>x.revisionId==='r2');
 assert.equal(JSON.stringify(failed),JSON.stringify(failedCoverage));
 assert.equal(failed.requested,true);assert.equal(failed.selected,true);
 assert.equal(failed.state,'failed');assert.ok(failed.diagnostic);
 const retained=loaded.annotations.flatMap(x=>x.facts).find(x=>x.kind==='provenance' && x.record.id===base.id)?.record;
 assert.equal(JSON.stringify(retained),JSON.stringify(base));
 assert.equal(expectedFreshness(retained,loaded),'stale');
 assert.equal(checkFreshness({...retained,freshness:'stale'},loaded),'stale');
 assert.throws(()=>checkFreshness({...retained,freshness:'fresh'},loaded),{assertion:'FRESHNESS.LABEL',field:'freshness'});
});
