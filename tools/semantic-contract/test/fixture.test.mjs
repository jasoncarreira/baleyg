...
async function writeJavaCorpus(root,{document,text,decls,calls,refs,facts,scenarios,assertions,negatives}) {
 ...
 const semanticRoles=['definition','read','write','call','type','import'];
 const coverage=producerId=>({producerId,language:'java',sourceSetId:'main',documentPath:document.path,revisionId:'r1',requested:true,selected:true,
  state:producerId==='native'?'partial':'complete',supportedRoles:producerId==='native'?['read','write','type']:semanticRoles,
  observedRoles:producerId==='native'?['read','write']:semanticRoles,diagnostic:producerId==='native'?'type not observed':null});
 ...
 const fixture={formatVersion:1,profile:'corpus',language:'java',sourceSets:[{id:'main',rootId:'root',languages:['java'],dependencies:[]}],producers,revisions:[revision],comparison:{sourceSetId:'main',revisionId:'r1',producers:producers.map(producer=>producer.id==='semantic'?{...producer,version:'2'}:producer)},coverageIntents:producers.map(producer=>({producerId:producer.id,document,revisionId:'r1',requestedRoles:producer.id==='native'?['read','write','type']:semanticRoles,measurementSupport:support})),nativeArtifact:'captures/native.json',semanticArtifacts:['captures/semantic.json'],annotationFiles:['snapshots/src/Corpus.java.annotations.json'],answersFile:'expected/answers.json',dispositionsFile:'expected/dispositions.json',anchorCasesFile:'expected/anchors.json',captures};
