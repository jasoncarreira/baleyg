  ...
  for (const row of rows) {
    for (const field of [
      ...
      if (!Object.hasOwn(row, field))
        throw new TypeError(`Control missing ${field}`);
    }
    if (
      typeof row.check !== "function" ||
      typeof row.baseline !== "function" ||
      typeof row.mutate !== "function"
    )
      throw new TypeError(
        `Control ${row.id} requires baseline, check and mutate functions`,
      );
    if (ids.has(row.id) || pending.has(row.id))
      throw new TypeError(`Duplicate control ${row.id}`);
    pending.add(row.id);
  }
  for (const row of rows) {
    ids.add(row.id);
...
export async function runControl(row) {
  ...
  assert.notEqual(
    await row.check(original),
    false,
    `${row.id}: baseline check failed`,
  );
  const changed = await row.mutate(structuredClone(original));
  await assert.rejects(
    async () => row.check(changed),
    (error) => {
      assert.equal(
        error.assertion,
...
export function schemaFieldControls(typeName, specimen) {
  ...
  if (!shape?.object)
    throw new TypeError("Schema field controls require an object");
  return registerControls(
    Object.keys(shape.object).map((field) => ({
      id: `FORMAT.${typeName}.${field}.missing`,
      baseline: () => {
        validate(typeName, specimen);
        return specimen;
      },
      mutate: (value) => {
        delete value[field];
      ...
      check: (value) => validate(typeName, value),
      expectedAssertion: "FORMAT.SHAPE",
      expectedCode: "invalidRecord",
      expectedField: `${typeName}.${field}`,
    })),
  );
}
