  ...
  async function walk(dir, prefix = "") {
    for (const entry of await readdir(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) await walk(join(dir, entry.name), path);
        ...
        files.push([path, contentHash(await readFile(join(dir, entry.name)))]);
    }
  }
  await walk(root);
...
test("AUTHOR.IDS computes native declaration, header and occurrence identities without answers", async (t) => {
  ...
  const recordsManifest = parseJson(
    await readFile(join(example, "generated/manifest.json")),
  );
  const records = parseJson(
    await readFile(join(example, "generated", recordsManifest.records.path)),
  );
  for (const row of ids.declarations) {
    const declaration = records.declarations.find(
      (x) =>
        x.syntaxId === row.syntaxId &&
    ...
    const siblings = records.declarations
      .filter(
        (x) =>
          x.revisionId === row.revisionId &&
          same(x.document, row.document) &&
          same(x.ancestors, row.ancestors) &&
          x.key.kind === row.key.kind &&
          x.key.name === row.key.name &&
    ...
    assert.equal(
      row.identicalHeaderCount,
      headers.filter((x) => x === row.headerHash).length,
      row.ref,
    );
    assert.equal(
      row.syntaxId,
  ...
  assert.deepEqual(
    await tree(root),
    before,
    "helper must not create or edit any fixture file",
  );
});
...
test("AUTHOR.HASHES prints expected and actual captured bytes without writing expected files", async (t) => {
  ...
  assert.deepEqual(
    hashes.captures.find((x) => x.ref === first.ref),
    {
      ref: first.ref,
      kind: first.kind,
      file: first.file,
      declaredHash: first.hash,
      actualHash: contentHash(Buffer.from("different captured bytes")),
      matches: false,
    },
  );
  for (const snapshot of hashes.snapshots) {
    ...
    for (const document of snapshot.documents)
      assert.equal(
        ...
        contentHash(await readFile(join(root, document.sourceFile))),
      );
  }
  let output = "";
  await main(["hashes", root], {
    write: (text) => {
      output += text;
    },
  });
