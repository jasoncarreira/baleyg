...
test('LOOKUP.LEXICAL decoding and language normalization preserve raw spelling identity',()=>{
  ...
  for (const language of ['java','javascript']) assert.notEqual(lookupKey(language,'é'),lookupKey(language,'é'));
  assert.equal(lookupKey('rust','é'),lookupKey('rust','é'));
  const make=name=>syntaxId({sourceSet:'core',path:'a.rs',language:'rust',ancestors:[],declaration:{kind:'function',name,signature:null,ordinal:0}});
...
test('ROLE.APPLICABILITY alias is absent in Java and requires definition on declaration',()=>{
  ...
  assert.throws(()=>validateRoles('java',['alias'],{site:'declaration'}),/ROLE.ORDER/);
  validateRoles('python',['definition','alias'],{site:'declaration'});
  assert.throws(()=>validateRoles('python',['alias'],{site:'declaration'}),/ROLE.ALIAS/);
