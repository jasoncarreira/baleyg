...
export class FormatError extends Error {
  constructor(path, message) {
    ...
    this.name = 'FormatError'; this.assertion = 'FORMAT.SHAPE'; this.code = 'invalidRecord'; this.field = path;
  }
}
const fail = (path, message) => { throw new FormatError(path, message); };
const plain = value => value !== null && typeof value === 'object' && !Array.isArray(value) && (Object.getPrototypeOf(value) === Object.prototype || Object.getPrototypeOf(value) === null);
const scalar = s => typeof s === 'string' && s.length > 0 && !/[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?<![\uD800-\uDBFF])[\uDC00-\uDFFF]/u.test(s);
function walk(spec, value, path) {
  ...
  if (typeof spec === 'string') {
    ...
    if (spec === 'uint' && Number.isSafeInteger(value) && value >= 0 && !Object.is(value, -0)) return;
    if (spec === 'boolean' && typeof value === 'boolean') return;
    if (spec === 'hash' && typeof value === 'string' && /^[a-f0-9]{64}$/.test(value)) return;
    if (spec === 'syntaxId' && typeof value === 'string' && /^sid:v1:[a-f0-9]{32}$/.test(value)) return;
    if (spec === 'occurrenceId' && typeof value === 'string' && /^occ:v1:[a-f0-9]{32}$/.test(value)) return;
    if (spec === 'path' && scalar(value) && !/[\\\0]/.test(value) && !value.startsWith('/') && value.split('/').every(p => p !== '' && p !== '.' && p !== '..')) return;
    fail(path, `expected ${spec}`);
  }
  if (spec.nullable) { if (value === null) return; return walk(spec.nullable, value, path); }
  if (spec.either) {
    for (const variant of spec.either) { try { walk(variant, value, path); return; } catch (error) { if (!(error instanceof FormatError)) throw error; } }
  ...
  if (spec.union) {
    ...
    if (!Object.hasOwn(value, tag)) fail(`${path}.${tag}`, 'missing discriminator');
...
export function validate(typeName, value) {
  if (!Object.hasOwn(schemas,typeName)) throw new TypeError(`Unknown format: ${typeName}`);
