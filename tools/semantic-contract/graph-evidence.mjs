...
const tupleKey = (producerId,document,revisionId) => key([producerId,...JSON.parse(docKey(document)),revisionId]);
...
function documentFor(row) {return {sourceSetId:row.sourceSetId,language:row.language,path:row.documentPath};}
function linkedFact(fact,proof,loaded,document,revisionId) {
 ...
 return proofId===proof.id && captured?.factRef===fact.ref && captured.factKind===fact.kind &&
  canonical(captured.fact)===canonical(fact) && proof.basis?.artifactHash===captured.hash &&
  docKey(proof.document)===docKey(document) && proof.revisionId===revisionId;
}
...
export function selectGraphEvidence(loaded,records,checked,result) {
 const revisionId=result.request.revisionId,producerId=result.request.semanticProducerId;
 ...
 for(const node of result.nodes){const declaration=node.declaration;addDocument(declaration.document);
  const k=docKey(declaration.document);if(!returned.has(k))returned.set(k,new Set());returned.get(k).add(declaration.syntaxId);
 }
 ...
 function addCoverage(id,document,revision){
  ...
  if(!row)fail('GRAPH.COVERAGE','coverage',`missing ${id}/${revision}/${document.path} tuple`);
  coverage.set(tupleKey(id,document,revision),row);return row;
 }
 ...
 for(const node of result.nodes)addProof(node.declaration.provenanceId,nativeId,node.declaration.document,revisionId);
 for(const edge of result.edges){
  ...
  if(edge.binding!==null){
   if(producerId===null||edge.call.revisionId!==revisionId||edge.binding.callId!==edge.call.id||
    ...
    fail('GRAPH.OCCURRENCE','edges.binding','old or foreign binding on current occurrence');
   addProof(edge.binding.provenanceId,producerId,edge.call.document,revisionId);
  }
 }
 if(producerId!==null){
  const chronology=loaded.revisionChronology.get(result.request.sourceSetId);
  const position=chronology?.findIndex(row=>row.id===revisionId)??-1;
  if(position<0)fail('GRAPH.CHRONOLOGY','request.revisionId','snapshot absent from admitted chronology');
  for(const [documentKey,syntaxIds] of returned){
   const document=documents.get(documentKey),current=coverageByTuple.get(tupleKey(producerId,document,revisionId));
   let selectedRevision=revisionId;
   if(['failed','omitted'].includes(current.state)){
    const previous=[...chronology.slice(0,position)].reverse().find(snapshot=>{
     const row=coverageByTuple.get(tupleKey(producerId,document,snapshot.id));
    ...
    if(!captured.selected)fail('GRAPH.HISTORY','coverage','historical tuple is not selected');
   }else if(!current.selected)continue;
   for(const annotation of loaded.annotations){
    if(annotation.revisionId!==selectedRevision||docKey(annotation.document)!==documentKey)continue;
    for(const fact of annotation.facts){
     if(!['declarationBinding','symbol','typeRelationship'].includes(fact.kind))continue;
     const normalized=records[fact.kind==='declarationBinding'?'declarationBindings':fact.kind==='symbol'?'symbols':'typeRelationships'];
     const proofId=fact.kind==='typeRelationship'?fact.provenanceRef:fact.record.provenanceId;
     ...
     if(!linkedFact(fact,proof,loaded,document,selectedRevision))
      fail('GRAPH.HISTORY','provenance','historical fact lacks captured producer/document proof');
     const names=normalized.some(row=>row.provenanceId===proofId&&(
      fact.kind==='declarationBinding'?row.syntaxId!==null&&syntaxIds.has(row.syntaxId):
      fact.kind==='symbol'?row.declarations.some(target=>target.kind==='internal'&&syntaxIds.has(target.syntaxId)&&docKey(target.document)===documentKey):
      row.source.kind==='internal'&&syntaxIds.has(row.source.syntaxId)&&docKey(row.source.document)===documentKey));
     if(!names)continue;
 ...
 const coverageRows=[...coverage.values()].sort((a,b)=>byteOrder(a.producerId,b.producerId)||
  byteOrder(a.sourceSetId,b.sourceSetId)||byteOrder(a.language,b.language)||
  byteOrder(a.documentPath,b.documentPath)||byteOrder(a.revisionId,b.revisionId));
 const proofRows=[...provenance.values()].sort((a,b)=>byteOrder(a.id,b.id));
 return {coverage:coverageRows,provenance:proofRows,
  selectedCoverageIncomplete:coverageRows.some(row=>row.selected&&['failed','partial'].includes(row.state))};
}

export function checkGraphEvidence(loaded,records,checked,actual,expectedTraversal) {
 if(!expectedTraversal)fail('GRAPH.TRAVERSAL','result','independent expected traversal required');
 const expected=selectGraphEvidence(loaded,records,checked,expectedTraversal);
 for(const field of ['coverage','provenance'])if(canonical(actual[field])!==canonical(expected[field]))
  fail(field==='coverage'?'GRAPH.COVERAGE':'GRAPH.PROVENANCE',field,`returned ${field} differs from source-derived evidence`);
 return expected;
}
