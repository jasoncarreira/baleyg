...
function warningKeys(result) {
  const keys = new Map();
  const add = (code, id) =>
    keys.set(JSON.stringify([code, id]), { code, provenanceId: id });
  ...
  for (const edge of result.edges) {
    ...
    if (binding.resolution === "ambiguous")
      add("bindingAmbiguous", binding.provenanceId);
  }
...
export function checkWarnings(result) {
  ...
  const expected = warningKeys(result),
    seen = new Set(),
    returned = new Set(result.provenance.map((row) => row.id));
  for (const edge of result.edges)
    if (edge.binding && !returned.has(edge.binding.provenanceId))
      fail(
        "WARNING.PROVENANCE",
        "edges.binding.provenanceId",
        "binding proof absent from returned provenance",
      );
  for (let i = 0; i < result.warnings.length; i++) {
    const warning = result.warnings[i],
      key = JSON.stringify([warning.code, warning.provenanceId]);
    if (
      !codes.includes(warning.code) ||
      typeof warning.message !== "string" ||
      !warning.message.length ||
      !(
        warning.provenanceId === null ||
        (typeof warning.provenanceId === "string" &&
          warning.provenanceId.length > 0)
      )
    )
      fail(
        "WARNING.SHAPE",
        `warnings[${i}]`,
        "invalid code, provenance or empty message",
      );
    if (seen.has(key))
      fail("WARNING.DUPLICATE", `warnings[${i}]`, "duplicate warning key");
    seen.add(key);
  }
  const actualKeys = result.warnings.map(({ code, provenanceId }) => ({
    code,
    provenanceId,
  }));
  if (
    canonicalBytes(actualKeys).toString("hex") !==
    canonicalBytes(expected).toString("hex")
  )
    fail(
      "WARNING.KEYS",
      "warnings",
      "missing, extra or enum-order misordered warning key",
    );
  return true;
}
