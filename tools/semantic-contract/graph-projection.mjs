...
export function graphProjection(records, request) {
  ...
  const requested = records.revisions.find(
    (r) => r.sourceSetId === request.sourceSetId && r.id === request.revisionId,
  );
  if (!requested) missing("revisionId");
  const captures = new Map(
    records.revisions.map((r) => [tuple(r.sourceSetId, r.id), r]),
  );
    ...
    records.declarations
      .filter(
        (d) =>
          ...
          d.document.sourceSetId === request.sourceSetId,
      )
      .map((d) => JSON.stringify([d.syntaxId, documentKey(d.document)])),
  );
  const comparison = records.comparison;
  const matchesComparison =
    comparison?.sourceSetId === request.sourceSetId &&
    comparison?.revisionId === request.revisionId;
  const producers = matchesComparison
    ? comparison.producers
    : records.producers;
  const document = (d) =>
    requested?.documents.find((row) => documentKey(row.key) === documentKey(d));
  function proof(row) {
    ...
    if (!wanted || wanted.contentHash !== row.contentHash) freshness = "stale";
      ...
      row.revisionId !== request.revisionId ||
      row.document.sourceSetId !== request.sourceSetId
    )
      freshness = "possiblyStale";
    else if (row.basis) {
      const basis = row.basis,
        producer = producers?.find((p) => p.id === basis.producerId);
      if (
        !producer ||
        producer.kind !== "semantic" ||
        !producer.languages.includes(basis.language) ||
        producer.version !== basis.producerVersion ||
        producer.executableHash !== basis.producerHash ||
        !same(
          producer.languages,
          records.producers.find((p) => p.id === basis.producerId)?.languages,
        ) ||
        producer.positionEncoding !==
          records.producers.find((p) => p.id === basis.producerId)
            ?.positionEncoding ||
        sourceManifestHash(
          requested.documents.map((d) => ({
            document: d.key,
            contentHash: d.contentHash,
          })),
        ) !== basis.sourceManifestHash ||
        requested.toolchainHash !== basis.toolchainHash ||
        requested.configHash !== basis.configHash ||
        requested.dependencyHash !== basis.dependencyHash
      )
        freshness = "possiblyStale";
    }
  ...
  function binding(row) {
    if (matchesComparison || row.declaredTarget?.kind !== "internal")
      return row;
    const target = row.declaredTarget,
      captured = captures
        .get(tuple(target.document.sourceSetId, target.revisionId))
        ?.documents.find(
          (d) => documentKey(d.key) === documentKey(target.document),
        );
    ...
    return {
      ...
      staleTarget:
        ...
        !declarations.has(
          JSON.stringify([target.syntaxId, documentKey(wanted.key)]),
        ),
    };
  }
