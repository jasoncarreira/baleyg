...
export function toByteRange(source, range) {
  ...
  const { tables } = scalarTable(source),
    table = tables[range.encoding];
  if (
    range.start > range.end ||
    !table.has(range.start) ||
    !table.has(range.end)
  )
    throw new RangeError(
      "COORD.INVALID_RANGE invalidRange: offset outside source or scalar boundary",
    );
...
export function verifyWitness(source, witness, { within = null } = {}) {
  ...
  if (within && (range.start < within.start || range.end > within.end))
    throw new RangeError("WITNESS.CONTAINMENT witness outside invocation");
  if (bytes.subarray(range.start, range.end).toString("utf8") !== witness.text)
    throw new Error("WITNESS.BYTES source spelling differs");
  return range;
}
...
export function verifyCallSpelling(source, call, witness) {
  if (call.calleeRange !== null || call.spelling === null)
    throw new TypeError("WITNESS.CALL requires spelling and null calleeRange");
  if (witness == null)
    throw new Error("WITNESS.MISSING separate spelling witness required");
  if (witness.text !== call.spelling)
    throw new Error("WITNESS.BYTES source spelling differs");
