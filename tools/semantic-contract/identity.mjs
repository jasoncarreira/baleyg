...
const domains = Object.freeze({syntax:'baleyg.syntax.v1\0',occurrence:'baleyg.occurrence.v1\0',header:'baleyg.header.v1\0',siblingGroup:'baleyg.sibling-group.v1\0'});
const shapes = Object.freeze({syntax:'SyntaxDigestInput',occurrence:'OccurrenceDigestInput',header:'HeaderDigestInput',siblingGroup:'SiblingGroupInput'});
export function digest(kind, input) {
  if (!Object.hasOwn(domains,kind)) throw new TypeError(`IDENTITY.DOMAIN unknown ${kind}`);
  validate(shapes[kind],input);
  return createHash('sha256').update(domains[kind]).update(canonicalBytes(input)).digest('hex');
}
...
export function sourceManifestHash(rows) {
  ...
  for (const row of rows) {
    ...
    if (seen.has(identity)) throw new Error('IDENTITY.MANIFEST duplicate document');
    seen.add(identity);
    const tuple = [languages.indexOf(key.language),Buffer.from(key.path)];
    if (previous && (previous[0] > tuple[0] || (previous[0] === tuple[0] && Buffer.compare(previous[1],tuple[1]) >= 0))) throw new Error('IDENTITY.MANIFEST unsorted documents');
    previous = tuple;
  }
  return contentHash(canonicalBytes(rows));
}
export const syntaxId = input => `sid:v1:${digest('syntax',input).slice(0,32)}`;
export const occurrenceId = input => `occ:v1:${digest('occurrence',input).slice(0,32)}`;
export const headerHash = header => digest('header',header);
export const siblingGroupHash = headers => digest('siblingGroup',{headers});
...
export function identityRegistry(hash = digest) {
  ...
  function register(kind,input) {
    if (kind !== 'syntax' && kind !== 'occurrence') throw new TypeError('IDENTITY.DOMAIN expected syntax or occurrence');
    validate(shapes[kind],input);
    const value = hash(kind,input);
    if (!/^[0-9a-f]{64}$/.test(value)) throw new TypeError('IDENTITY.HASH expected full lowercase SHA-256');
    const id = `${kind === 'syntax' ? 'sid' : 'occ'}:v1:${value.slice(0,32)}`;
    const canonical = canonicalBytes(input).toString('hex');
    const old = seen.get(id);
    if (old !== undefined && old !== canonical) throw new Error(`IDENTITY.COLLISION ${id}`);
...
export function assignOrdinals(rows) {
  ...
  for (const group of groups.values()) {
    group.sort((a,b) => a.range.start-b.range.start || a.range.end-b.range.end);
    for (let i=0;i<group.length;i++) {
      if (i && group[i].range.start===group[i-1].range.start && group[i].range.end===group[i-1].range.end) throw new Error('IDENTITY.ORDINAL duplicate native range');
...
export function assignOccurrenceOrdinals(rows) {
  ...
  for (const group of groups.values()) {
    group.sort((a,b)=>a.range.start-b.range.start || a.range.end-b.range.end);
    for (let i=0;i<group.length;i++) {
      if (i && group[i].range.start===group[i-1].range.start && group[i].range.end===group[i-1].range.end) throw new Error('IDENTITY.OCCURRENCE duplicate native range');
      result.set(group[i],i);
    }
  }
  return result;
